...
export function TimetableCard({ lessons, title, date, notice }: TimetableCardProps) {
  ...
  const hasLessons = Object.keys(lessonsByPeriod).length > 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3">
        {hasLessons ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          <div className="flex items-center gap-2">
                            <BookOpen
                              className={`h-5 w-5 ${
                                lesson.isSubstitution ? "text-destructive" : "text-primary"
                              }`}
                            />
                            <h3
          ...
          <Card className="border-2 border-dashed border-muted-foreground/40 bg-muted/40">
            <div className="p-6 text-center space-y-2">
              <div className="text-lg font-semibold">
                {notice || "Kein Stundenplan verfügbar."}
              </div>
              <p className="text-sm text-muted-foreground">
                Sobald neue Pläne verfügbar sind, werden sie hier angezeigt.
              </p>
            </div>
          </Card>
        )}
